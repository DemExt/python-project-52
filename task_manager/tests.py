...
class TasksTest(TestCase):
    ...
    def test_delete_task_by_non_author(self):
        ...
        self.assertTrue(
            any(
                "Задачу может удалить только ее автор" in m.message
                for m in messages
            )
        )
...
class LabelsTest(TestCase):
    ...
    def test_label_crud(self):
        ...
        task = Task.objects.create(
            name="T", status=self.status, author=self.user, executor=self.user
        )
...
def test_filter_self_tasks(self):
    ...
    Task.objects.create(
        name="Other Task", author=other_user, status=self.status
    )
